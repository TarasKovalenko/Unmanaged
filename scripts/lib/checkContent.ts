...
// Depends only on the content: no SDK or machine details, so CI and every
// contributor generate the same bytes.
export function generatedCSharpJson(snippets: GeneratedSnippets): string {
  return JSON.stringify({ note: 'Generated by npm run check:content. Do not edit.', snippets }, null, 2) + '\n';
}
...
export async function csharpPass(deps: Deps): Promise<boolean> {
  ...
  await fs.writeFile(GENERATED_CSHARP, generatedCSharpJson(report.snippets));
  const { counts } = report;
